...
def precision_top_k_day(df_day, top_k=100):
    ...
    """
    if top_k is None:
        top_k = df_day['is_fraud'].sum()  # Default to the number of fraud cases if not specified.

    # Order transactions by descending fraud prediction probabilities.
    df_day = df_day.sort_values(by="predictions", ascending=False).reset_index(drop=True)

    # Select the top K most suspicious transactions.
    df_day_top_k = df_day.head(top_k)

    # Identify detected fraudulent transactions.
    detected_fraudulent_transactions = df_day_top_k[df_day_top_k['is_fraud'] == 1]['trans_num'].tolist()
...
def precision_top_k(predictions_df, top_k=100):
    ...
    for day in unique_days:
        df_day = predictions_df[predictions_df['trans_date'] == day][['trans_num', 'is_fraud', 'predictions']]
    ...
    return num_fraudulent_transactions_per_day, precision_top_k_per_day_list, mean_precision_top_k