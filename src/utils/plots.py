...
class FraudMap:
    ...
    def __init__(self, fraud_transactions):
        ...
        self.fraud_transactions["cc_num"] = (
            self.fraud_transactions["cc_num"].astype(str).str.strip()
        )
    ...
    def add_flower_pattern(self, transactions):
        ...
        for (lat, long), group in grouped:
            ...
            if n > 1:
                ...
                for i, row in enumerate(group.itertuples()):
                    new_coords.append(
                        (row.Index, lat + lat_offsets[i], long + long_offsets[i])
                    )
            else:
        ...
        coords_df = pd.DataFrame(
            new_coords, columns=["index", "lat_adjusted", "long_adjusted"]
        ).set_index("index")
        ...
        if show_cardholders:
            ...
            fig.add_trace(
                go.Scattermapbox(
                    ...
                    customdata=self.fraud_transactions[
                        ["amt", "city", "job", "first", "last"]
                    ],
                    hovertemplate=(
    ...
    def setup_callbacks(self):
        @self.app.callback(
            Output("fraud-map", "figure"), Input("toggle-checklist", "value")
        )
        ...
        @self.app.callback(
            Output("click-data", "children"), Input("fraud-map", "clickData")
        )
