...
def fit_model(classifier, train_df, input_features, output_feature="is_fraud", 
              scale=False, save=False, save_model_name=None):
    """
    ...
    if save and save_model_name:
        save_path = os.path.join("..", "saved_model", "classifier", f"{save_model_name}.pkl")
...
def evaluate_model_on_splits(splits, classifier, input_features, top_k_list=[100], summary="Default"):
    """
    ...
    for split in splits:
        ...
        perf_train = performance_assessment(train_df, top_k_list=top_k_list, rounded=False)
        perf_train.columns += " train"

        performances_df_folds = pd.concat([performances_df_folds, 
                                           pd.concat([perf_val, perf_train], axis=1)], 
                                          ignore_index=True)
