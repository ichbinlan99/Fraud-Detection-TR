...
def categorize_jobs(df, job_col, job_to_category, encoding='label', default_category=None):
    """
...
def customer_spending_behaviour(df, window_lengths=[180]):
    """
    ...
    - amount_over_average_[window_size]: Amount over average for different window sizes.
    ...
    for window_length in window_lengths:
        # Calculate non-fraudulent rolling average distance
        df[f"avg_distance_{window_length}_days"] = df[df['is_fraud']==0].sort_values(by=["trans_date_trans_time"]).groupby("cc_num").apply(
        ...
        # Calculate non-fraudulent rolling average amount
        df[f"sum_amount_{window_length}_days"] = df[df['is_fraud']==0].sort_values(by=["trans_date_trans_time"]).groupby("cc_num").apply(
            lambda x: x.rolling(f'{window_length}D', on='trans_date_trans_time')["amt"].sum().shift(1)
        ).reset_index(level=0, drop=True)

        df[f"count_amount_{window_length}_days"] = df[df['is_fraud']==0].sort_values(by=["trans_date_trans_time"]).groupby("cc_num").apply(
...
def get_merchant_risk_rolling_window(transactions, delay_period=7, window_size=[1, 7, 30]):
    ...
    transactions = transactions.reset_index()  # Reset index to original DataFrame index
