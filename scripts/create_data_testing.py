...
from data.data_transformation import (
    load_data, encode, categorize_and_encode_jobs, assign_age_group,
    calculate_distance, generic_customer_spending_behaviour,
    general_customer_bahaviour, get_merchant_risk_rolling_window
)
...
def preprocess_data():
    ...
    test_df = categorize_and_encode_jobs(test_df, 'job', job_categories, 'saved_model/encoders/job_encoder.pkl')
    logger.info("Jobs categorized and encoded successfully.")
    
    # Load and apply merchant category encoder
    merchant_cat_encoder = load_pickle_model("saved_model/encoders/merchant_cat_encoder.pkl")
    ...
    age_labels = [1, 0, 2, 3]
    test_df = assign_age_group(test_df, "dob", "trans_date_trans_time", age_bins, age_labels)
    ...
    test_df = generic_customer_spending_behaviour(test_df, window_lengths=[1, 7, 30, 90, 180])
    test_df = general_customer_bahaviour(test_df)
    ...
    test_df["city_size"] = pd.cut(test_df["city_pop"], bins=city_size_bins, labels=city_size_labels).astype("int64")
